...
const DraftLeagueCourt: FC<DraftLeagueCourtProps> = ({
  ...
  const handleCoachClick = (myCoach: CoachView) => {
    if (selectedFreePlayerCoach) {
      ...
      toast.info("Select a free coach to swap with!");
    }
  };
  ...
  const triggerSwap = (clickedPlayer: PlayerView) => {
    ...
    if (selectedTeamPlayer) {
      ...
      if (
        teamLineup &&
        clickedPlayer.playerId === teamLineup.captain.playerId
      ) {
        toast.error(
          "You cannot switch the captain! Please assign captaincy to another player first.",
        );
        setSelectedTeamPlayer(null);
        return;
      }
  ...
  const isHighlight = (p: PlayerView) => {
    ...
    if (selectedTeamPlayer && selectedTeamPlayer.playerId !== p.playerId) {
      return selectedTeamPlayer.position === p.position;
    }
